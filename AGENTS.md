...
This repository is a notebook-based news crawling and text-analysis pipeline, not a packaged application. Core workflow notebooks live in `notebooks/crawling/`. Raw and intermediate crawl artifacts are written to `data/crawling/`, weekly press-analysis inputs live in `data/news/`, reusable resources such as sentiment lexicons live in `resources/`, and generated charts/tables belong in `outputs/`. Keep top-level notes such as `README.md`, `docs/분석_진행상황.md`, and method guides in Markdown.

## Build, Test, and Development Commands
There is no build step or CLI entrypoint. Run notebooks top-to-bottom in Jupyter or Colab:

- `jupyter lab notebooks/crawling` opens the local notebook workspace.
- `git status --short` checks notebook and data-file diffs before committing.
- `rg "query_ranges|KEEP_TERMS|NEWS_STOP" notebooks/crawling` finds the main configuration blocks quickly.
...
## Testing Guidelines
There is no automated test suite or coverage gate. Validate changes by rerunning the affected notebook on a narrow date range or one query, then confirm output schema, row counts, and resume files in `data/crawling/`. For preprocessing or tokenization edits, spot-check columns such as `body_cleaned`, `tokens`, `source_period`, and duplicate flags before trusting downstream analysis.
